...
describe.skipIf(!HAS_KEYS)('POST /knowledge/ask (uctan uca, gercek modeller)', () => {
  ...
  it(
    'BAGLAMDAKI nota dayali dogru cevap verir ve kaynagi bildirir',
    async () => {
      ...
      await addNote(token, 'Sunucularimiz Kubernetes uzerinde calisiyor ve Argo CD ile deploy ediliyor.');
  ...
  it(
    'BAGLAMDA OLMAYAN soruda UYDURMAZ, yonlendirir',
    async () => {
      const token = await signInAs('owner', 'owner@example.com');
  ...
  it(
    'ilk soruda YENI konusma acar ve id sini doner',
    async () => {
      const token = await signInAs('owner', 'owner@example.com');
  ...
  it(
    'soru ve cevap messages tablosuna yazilir',
    async () => {
      const token = await signInAs('owner', 'owner@example.com');
      await addNote(token, 'Sirketimiz yazilim gelistiriyor.');
  ...
  it(
    'IKINCI soruda conversationId ile GECMIS gercekten kullanilir',
    async () => {
      const token = await signInAs('owner', 'owner@example.com');
      await addNote(
        token,
        'Muhasebe ekibimiz her ayin son is gunu fatura kesiyor. Sureci Ayse Yilmaz yonetiyor.',
      );

      const first = await ask(token, { question: 'Fatura surecini kim yonetiyor?' });
      expect(first.status).toBe(200);

      // Takip sorusu: "o" zamiri YALNIZCA gecmis varsa cozulebilir. Gecmis
      // gecirilmeseydi model neyi sordugumuzu bilemezdi.
      const second = await ask(token, {
        question: 'Peki o kisi hangi ekipte calisiyor?',
  ...
  it(
    'BASKA tenant in notu kaynak olarak GELMEZ (RLS)',
    async () => {
